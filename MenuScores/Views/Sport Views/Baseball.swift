                        ...
                        Menu(date) {
                            ForEach(gamesForDate, id: \.id) { game in
                                Menu {
                                    ...
                                    HStack {
                                        AsyncImage(
                                            url: URL(string: game.competitions.first?.competitors?[safe: 1]?.team?.logo ?? "https://a.espncdn.com/combiner/i?img=/redesign/assets/img/icons/ESPN-icon-baseball.png&h=160&w=160&scale=crop&cquality=100")
                                        ) { image in
