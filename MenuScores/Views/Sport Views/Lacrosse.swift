            ...
            if !viewModel.games.isEmpty {
                ForEach(Array(viewModel.games.enumerated()), id: \.1.id) { _, game in
                    Menu {
                                ...
                                Task {
                                    ...
                                    if NotchViewModel.shared.notch == nil {
                                        let newNotch = DynamicNotch(
                                            ...
                                            Info(notchViewModel: notchViewModel, sport: "Lacrosse", league: "\(league)")
                                        } compactLeading: {
                        ...
                        HStack {
                            AsyncImage(
                                url: URL(string: game.competitions[0].competitors?[1].team?.logo ?? "https://a.espncdn.com/combiner/i?img=/redesign/assets/img/icons/ESPN-icon-lacrosse.png&h=80&w=80&scale=crop&cquality=40")
                            ) { image in
